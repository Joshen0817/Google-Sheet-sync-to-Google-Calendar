...
function applyFormattingAndValidation_() {
  ...
  const filtered = rules.filter(r => r.getDescription && r.getDescription() !== 'Row highlight when DELETE');
  const delRule = SpreadsheetApp.newConditionalFormatRule()
    .whenFormulaSatisfied(`=$${statusColLetter}2="DELETE"`)
    .setBackground('#FDECEC')
    .setFontColor('#B00020')
    .setRanges([dataRange])
    .setDescription('Row highlight when DELETE')
    .build();
  filtered.push(delRule);
...
/***** 把「初始化」加入選單（替換你現有 onOpen 內的 createMenu 區塊或在後面多加一行）*****/
(function patchMenu_(){
  // 重新建立選單（避免重複加入）
  const ui = SpreadsheetApp.getUi();
  const menu = ui.createMenu('Calendar 同步')
    .addItem('重推全表（Sheet → Calendar）','syncAllRows')
    .addSeparator()
    .addItem('同步目前列（不需編輯）','syncActiveRow')      // 若你沒加這支，可刪掉這行
    .addSeparator()
  ...
  try { menu.addToUi(); } catch(e) { /* onOpen 尚未執行時忽略 */ }
})();
